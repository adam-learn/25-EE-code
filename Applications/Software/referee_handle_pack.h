...
typedef struct // 0x0001  比赛状态数据11
{
...
? 200：200 颗弹丸
***************************/
typedef struct // 0x0102  补给站动作标识数据
{
	uint8_t reserved;
...
bit 7-15：保留
***/
typedef struct // 0x0105  飞镖发射时间数据
{
...
? bit 3-7： 保留
***************/
typedef struct // 0x201  机器人性能体系数据
{
...
typedef struct // 0x0205  空中支援时间数据
{
	uint8_t airforce_status;
	uint8_t time_remain;
} ext_air_support_data_t;

typedef struct // 0x0206  伤害状态数据
{
...
? bit19-31：保留
*****************/
typedef struct // 0x0209  机器人 RFID 状态
{
...
*************/
typedef struct // 0x020B  地面机器人位置数据
{
	float hero_x;
	float hero_y;
	float engineer_x;
...
typedef struct // 0x020C  雷达标记进度数据
{
...
? 1：对手正在被触发双倍易伤*/
/***/
typedef struct // 0x020E 雷达是否具有双倍易伤资格
{
	uint8_t radar_info;
} ext_radar_info_t;
...
推。
**/
typedef struct // 0x0120  哨兵自主决策指令
{
	uint32_t sentry_cmd;
} ext_sentry_cmd_t;

/**/
typedef struct // 0x0121 雷达自主决策指令
{
	uint8_t radar_cmd;
} ext_radar_cmd_t;

typedef struct // 0x0301  机器人交互数据
{
...
/***/
typedef struct // 0x0307  选手端小地图接收哨兵数据
{
	uint8_t intention;
...
/***/
typedef struct // 0x0308  己方机器人可通过常规链路向己方任意选手端发送自定义的消息，该消息会在己方选手端特定位置显示。
{
	uint16_t sender_id;
	uint16_t receiver_id;
...
typedef struct // 0x0302  自定义控制器与机器人交互数据
{
	int16_t  Impel_distance;
	int16_t  Traverse_distance;
	int16_t  Lifting_distance;
	int16_t  Pitch_Root;
	int16_t  Roll_Root;
	int16_t  Pitch_end;
	int16_t  Roll_end;
	uint8_t	 KEY_value;
	uint8_t  unknowchar[15];
//float test_float[7];
//short unknow;	
	
} ext_custom_robot_data_t;

typedef struct // 0x0304  键鼠遥控数据
{
	int16_t mouse_x;
	int16_t mouse_y;
	int16_t mouse_z;
...
typedef struct // 0x0306  自定义控制器与选手端交互数据
{
	uint16_t key_value;
